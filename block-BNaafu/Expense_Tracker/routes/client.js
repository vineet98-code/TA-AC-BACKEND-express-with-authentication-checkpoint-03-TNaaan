...
router.get('/statementList', (req, res, next) => {
    ...
    Expense.find({ userId: req.user.id,
        ...
        Income.find({
            ...
            res.render('incomeExpenseStatement', {expenses, incomes, currentMonth});
        })
    }) 
});
...
router.get('/statementList/filterByDate', (req, res, next) => {
    ...
    Income.find({ userId: req.user.id, date: {
        $gte: new Date(startDate),
        $lt: new Date(endDate)
    }}, (err, incomes) => {
        if(err) return next(err);
        Expense.find({ userId: req.user.id, date: {
            $gte: new Date(startDate),
            ...
            res.render('incomeExpenseStatement', {incomes, expenses, balance, currentMonth});
        })
    })
});
    ...
    Income.find({date: {
        ...
        Expense.find({date: {
            ...
            if(err) return next(err);
                ...
                let sumOfIncomes = incomes.reduce((acc, curr) => acc + Number(curr.amount), 0);
                let balance = sumOfIncomes - sumOfExpenses;
                res.render('incomeExpenseStatement', {incomes, expenses, balance: balance, currentMonth: currentMonth});
        })
    })
